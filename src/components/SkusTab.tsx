      ...
      <div className="flex justify-between items-end">
        ...
        <div className="flex gap-3 items-center">
          ...
          <button 
            onClick={() => {
              setEditingSku(null);
              setSkuForm({ sku: '', price: 0, minStock: 10, pcsPerBox: 1, volumeLiters: 0, leadTimeDays: 0, needsFfBox: true });
          ...
          <tbody>
            {displayedSkus.map((s, index) => {
              ...
              <tr key={`${s.sku}-${index}`} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                ...
                <td className="px-6 py-4 text-center">
                  {s.needsFfBox !== false
                    ? <span className="px-2 py-1 rounded-lg text-xs font-bold bg-slate-100 text-slate-600">да</span>
                    : <span className="px-2 py-1 rounded-lg text-xs font-bold bg-amber-50 text-amber-600">нет</span>
                  }
                </td>
