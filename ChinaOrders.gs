...
function chinaBatchCost(batch, lines, rubCostsTotal, settings) {
  ...
  const out = list.map(function (l, i) {
    ...
    const s = boxStats[i];
    // Owner, 2026-09-24: ruble equivalents of the line's three currency components, worked out
    // BEFORE chinaLevelGroups touches costRub — so a group of lines can still show a levelled
    // costRub that differs a kopeck from goodsRub + chinaShareRub + freightShareRub, on purpose:
    // these three are the line's OWN money, the levelled costRub is the group's money reshared.
    // freightShareRub is the remainder, not a third multiplication, so the three always sum to
    // round2(cny × rubRate) exactly, whatever the rounding of the other two did.
    const goodsRub = rubRate > 0 ? roundToTwo(goods[i] * rubRate) : 0;
    const chinaShareRub = rubRate > 0 ? roundToTwo(chinaShares[i] * rubRate) : 0;
    const cnyRub = rubRate > 0 ? roundToTwo(cny * rubRate) : 0;
    const freightShareRub = rubRate > 0 ? roundToTwo(cnyRub - goodsRub - chinaShareRub) : 0;
    return {
      sumCny: goods[i],
...
function chinaBorrowedRate(batch, others) {
  const own = (others || []).filter(function (b) {
    return (b.rubRateSource === 'оплаты' || b.rubRateSource === 'вручную') && Number(b.rubRate) > 0;
  });
...
function chinaWithPaymentRate(ss, batch) {
  ...
  if (fromPayments > 0) {
    out.rubRate = fromPayments;
    out.rubRateSource = 'оплаты';
    out.rubRateFrom = '';
    return out;
  }
  ...
  out.rubRate = source ? source.rubRate : 0;
  out.rubRateSource = source ? 'предыдущая партия' : '';
  out.rubRateFrom = source ? source.code : '';
  return out;
}
...
function chinaRecostBorrowers(ss, username, skipIds) {
  ...
  batches.forEach(function (b) {
    ...
    if (b.rubRateSource === 'предыдущая партия' || b.rubRateSource === '') recalcChinaBatch(ss, b.id, username);
  });
}
